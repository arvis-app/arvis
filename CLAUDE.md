...
11. **Apple Touch Icon (iOS share sheet)** : doit être full-bleed — robot blanc sur fond orange `#D94B0A`, sans padding, 180×180px. Source : `icon-180.png` (fond transparent) recolorisé en blanc, composité sur fond `#D94B0A`. Fichier : `public/apple-touch-icon-v2.png`, référencé avec `sizes="180x180"` dans `<head>`. Safari met l'icône en cache agressivement — changer le nom de fichier pour invalider le cache.
